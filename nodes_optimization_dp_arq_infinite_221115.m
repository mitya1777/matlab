...
x_i = 10 : 1 : 30;                                                         % sensor node quantity
...
s = 20;                                                                     % the distributed resource quantity
s_k = 0;                                                                   % distributed among the rest area resource quantity
x_k = 0;                                                                   % the k-area distributed resource quantity
t_del = zeros(5, 21);
nodes = zeros(5, 21);                                                       % optimal node quantity matrix

t_del(5, :) = t_del_i(5, :);                                               % straight copy for the edge LWSN area

for s_k = (0 : 1 : s)                                                      % distributed resource enumeration loop
    for area = (4 : -1 : 1)                                                % area enumeration loop
...
distribution = zeros(1 , 5);                                               % final nodes distribution matrix creation
balance = s;                                                               % nodes remainder variable

for k = (1 : 1 : 5)                                                        % the depth-first search loop
	distribution(1, k) = nodes(k, balance + 1);
    balance = balance - distribution(1, k);
end
distribution(1, 5) = balance;
